...
class Solution:
    def exist(self, board: List[List[str]], word: str) -> bool:
        """DFS from every cell, matching the word one letter at a time and
        marking visited cells in place so a path can't reuse a cell."""

        # Time: O(m * n * 4^L) for word length L   Space: O(L) recursion depth
