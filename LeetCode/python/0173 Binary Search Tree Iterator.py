...
class BSTIterator:
    """Flatten the BST to a sorted list with an in-order traversal up front,
    then serve elements by index."""

    # __init__: O(n)   next/hasNext: O(1)   Space: O(n)

    def __init__(self, root: Optional[TreeNode]):
        def in_order(root: Optional[TreeNode]) -> List[int]:
            return (
                in_order(root.left) + [root.val] + in_order(root.right) if root else []
            )
    ...
    def case(
        ...
        for action, param in zip(actions, params):
            if action == "BSTIterator":
                s = BSTIterator(
                    BinarySearchTree.from_level_order_sequence(param[0]).root
                )
                actual.append(None)
            elif action == "next":
