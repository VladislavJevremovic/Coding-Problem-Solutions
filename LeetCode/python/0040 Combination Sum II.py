...
class Solution:
    def combinationSum2(self, candidates: List[int], target: int) -> List[List[int]]:
        """Backtrack over distinct values with their frequencies so each number
        is used at most its count, avoiding duplicate combinations."""
        ...
        def backtrack(combination, remainder, current, counter, result):
            ...
            for next_current in range(current, len(counter)):
                ...
                backtrack(
                    combination, remainder - candidate, next_current, counter, result
                )
