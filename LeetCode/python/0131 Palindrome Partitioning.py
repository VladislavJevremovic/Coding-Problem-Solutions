...
class Solution:
    def partition(self, s: str) -> List[List[str]]:
        """Backtrack over every cut point, extending the current partition only
        when the next prefix is a palindrome."""
        ...
        def backtrack(start: int, result: List[List[str]], current: List[str], s: str):
            ...
            for end in range(start, len(s)):
                if is_palindrome(s, start, end):
                    ...
                    backtrack(
                        end + 1, result, current, s
                    )  # continue with rest of s (after 1st substring)
                    current.pop()
