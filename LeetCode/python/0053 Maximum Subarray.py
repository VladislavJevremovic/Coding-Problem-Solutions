...
class Solution:
    def maxSubArray(self, nums: List[int]) -> int:
        """Kadane's: carry a running best-ending-here sum, restarting it whenever
        extending would hurt, and track the global max."""
        # Time: O(n)   Space: O(1)
        if not nums:
            return 0
...
class Solution2:
    def maxSubArray(self, nums: List[int]) -> int:
        """Divide and conquer: the best subarray lies entirely left, entirely
        right, or crosses the midpoint; recurse and combine."""
