...
class Solution:
    def solve(self, board: List[List[str]]) -> None:
        """DFS from every border 'O' to mark escaped regions, then flip the
        unmarked 'O's (truly surrounded) to 'X'."""
        ...
        # border cells
        borders = list(product(range(rows), [0, cols - 1])) + list(
            product([0, rows - 1], range(cols))
        )
