...
class Solution1:
    def minPathSum(self, grid: List[List[int]]) -> int:
        """Top-down DFS with memoization: each cell's cost is itself plus the
        cheaper of going down or right toward the bottom-right corner."""
        ...
        def dfs(grid, sums, x, y):
            ...
            sums[x][y] = min(
                min(infinity, dfs(grid, sums, x + 1, y) + grid[x][y]),
...
class Solution2:
    def minPathSum(self, grid: List[List[int]]) -> int:
        """Bottom-up DP done in place: add to each cell the smaller of its top
        or left neighbor's accumulated cost."""
        # Time: O(m * n)   Space: O(1) (mutates grid)
        m = len(grid)  # h
        if not m:
            return 0
        n = len(grid[0])  # w
