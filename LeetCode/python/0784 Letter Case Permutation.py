...
class Solution:
    def letterCasePermutation(self, s: str) -> List[str]:
        """Iteratively build all words, doubling the working set at every letter
        to branch into its lower and upper case (k = number of letters)."""
        ...
        for c in s:
            ...
            if c.isalpha():
                for i in range(n):
                    ...
                    r[n + i].append(
                        c.upper()
                    )  # add upper(c) to each word in second half
            else:
                for i in range(n):
