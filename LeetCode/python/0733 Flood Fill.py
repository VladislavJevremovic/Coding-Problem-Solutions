...
class Solution:
    def floodFill(
        ...
        """DFS from the start cell, recoloring every 4-directionally connected
        cell that shares the original color."""

        # Time: O(m * n)   Space: O(m * n)
        def floodFillRec(
            image: List[List[int]], sr: int, sc: int, newColor: int, targetColor: int
        ) -> List[List[int]]:
