...
class Solution:
    def findNumberOfLIS(self, nums: List[int]) -> int:
        """DP tracking, per index, the longest increasing run ending there and
        how many such runs; sum the counts over all maximal-length endings."""
            ...
            for j in range(i):
                if nums[i] > nums[j]:
                    if lis[j] + 1 > lis[i]:  # strictly increasing
                        ...
                        lis[j] + 1 == lis[i]
                    ):  # more same-length subsequences ending at lis[i]
                        count[i] += count[j]
