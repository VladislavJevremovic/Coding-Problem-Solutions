...
class WordDictionary:
    """Trie supporting wildcard search: '.' matches any single character, so a
    matching query forks a DFS over every child at that level. L = word length."""
    ...
    def search(self, word: str) -> bool:
        # Time: O(L) no wildcards, up to O(26^L) all wildcards   Space: O(L)
...
def test():
    def case(
        actions: List[str], params: List[List[str]], expected: List[Optional[bool]]
    ) -> bool:
        actual = []
        s = None
