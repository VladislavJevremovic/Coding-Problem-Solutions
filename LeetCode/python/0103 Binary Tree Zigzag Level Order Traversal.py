...
class Solution1:
    def zigzagLevelOrder(self, root: TreeNode) -> List[List[int]]:
        """BFS level by level, reversing every other level so values alternate
...
class Solution2:
    def zigzagLevelOrder(self, root: TreeNode) -> List[List[int]]:
        """BFS with a list-based queue, toggling a flag to reverse alternate
        levels."""
        # Time: O(n)   Space: O(n)
        levels = []
        Q = [root]
...
def test():
    def case(list1: List[int], expected: List[List[int]]) -> bool:
        ...
        return (
            Solution1().zigzagLevelOrder(tree.root) == expected
            if tree
            else not expected
        )
