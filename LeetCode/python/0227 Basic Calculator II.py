...
class Solution:
    def calculate(self, s: str) -> int:
        """Single pass tracking the pending operator: push +/- numbers onto a
        stack and fold * / into the stack top immediately, then sum the stack."""
        # Time: O(n)   Space: O(n)
        if not s:
            return 0
        ...
        for i, c in enumerate(s):
            ...
            if (
                c in ["+", "-", "*", "/"] or i == n - 1
            ):  # new number is found, apply previous operation
                if operation == "-":
