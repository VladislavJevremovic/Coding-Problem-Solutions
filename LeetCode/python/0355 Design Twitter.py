...
class Twitter:
    """Per-user follow sets and tweet lists with a global counter giving every
    tweet a monotonic timestamp, so a feed is the 10 newest tweets across the
    user and everyone they follow. F = followees, T = their tweets."""
    ...
    def getNewsFeed(self, userId: int) -> List[int]:
        # Time: O(N log N) over N = total followed tweets   Space: O(N)
        self.followed_users[userId].add(userId)

        tweets = []
        for f in self.followed_users[userId]:
            tweets.extend(self.tweets[f])
