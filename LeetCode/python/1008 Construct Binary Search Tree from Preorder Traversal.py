...
class Solution:
    def bstFromPreorder(self, preorder: List[int]) -> TreeNode:
        """Build the BST with a monotonic stack: each value becomes the left child
        of the stack top, or the right child of the last popped smaller ancestor."""
        # Time: O(n)   Space: O(n)
        size = len(preorder)
...
def test():
    def case(preorder: List[int], expected) -> bool:
        result = Solution().bstFromPreorder(preorder)
        return BinarySearchTree(result).to_level_order() == expected

    assert case([8, 5, 1, 7, 10, 12], [8, 5, 10, 1, 7, None, 12])
    assert case([1, 3], [1, None, 3])
