...
class Solution:
    def gameOfLife(self, board: List[List[int]]) -> None:
        """Encode next state in the high bit of each cell while reading the live
        count from the low bit, so the update is in place; then shift each cell
        right to reveal the next generation."""
        ...
        for i in range(m):
            for j in range(n):
                ...
                for k in range(len(x)):
                    nx = i + x[k]
                    ny = j + y[k]
                    if 0 <= nx < m and 0 <= ny < n and (board[nx][ny] & 1 == 1):
                        c += 1
