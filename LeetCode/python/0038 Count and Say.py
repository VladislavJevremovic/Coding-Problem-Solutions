...
class Solution:
    def countAndSay(self, n: int) -> str:
        """Start from "1" and apply the run-length "say" transform n-1 times,
        each term describing the previous one."""
        # Time: O(n * L) where L is the longest term length   Space: O(L)
        s = "1"
        for _ in range(1, n):
            s = self.say(s)
    ...
    def say(self, s: str) -> str:
        ...
        while i < len(sa):
            count = 1
            while (
                i + 1 < len(sa) and sa[i] == sa[i + 1]
            ):  # leave space ahead for actual digit, sum same occurrences
                i += 1
                count += 1
            r += str(count) + sa[i]
            i += 1
