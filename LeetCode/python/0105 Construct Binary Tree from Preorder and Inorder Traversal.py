...
class Solution:
    def buildTree(self, preorder: List[int], inorder: List[int]) -> Optional[TreeNode]:
        """Consume preorder front-to-back to pick each root, using the inorder
        value as a bound to know when a subtree ends."""

        # Time: O(n^2)   Space: O(n)  (pop(0) on the lists is O(n) each)
