...
class Solution:
    def minRemoveToMakeValid(self, s: str) -> str:
        """Stack the indices of unmatched parentheses, then rebuild the string
        skipping every index marked for removal."""
        # Time: O(n)   Space: O(n)
...
def test():
    s = Solution()
    assert s.minRemoveToMakeValid("lee(t(c)o)de)") in [
        "lee(t(c)o)de",
        "lee(t(co)de)",
        "lee(t(c)ode)",
    ]
